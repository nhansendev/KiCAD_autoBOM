...
def get_BOM_items(
    ...
    df = pd.DataFrame(
        columns=["Reference", "Value", "Description", "Footprint"] + additional_fields
    )
...
def _condense_df(df, replace_sequences=True, to_string=True):
    ...
    df.sort_values("Reference", ignore_index=True, inplace=True)
    # Group similar references into lists
    summary = (
        df.groupby(list(df.columns[df.columns != "Reference"].values))["Reference"]
        .apply(list)
        .reset_index()
    )
